...
author: Dharani Ganesan, Hussnain Khalid, Marc Braun
vignette: >
...
y_test_pred_1 <- pred(model_1, data = data_test) # error here
train_rmse <- sqrt(sum((y_train_pred_1 - data_train$medv)^2)/nrow(data_train))
test_rmse <- sqrt(sum((y_test_pred_1 - data_test$medv)^2)/nrow(data_test)) # error here
...
y_test_pred_2_a <- pred(model_2_a, data = data_test) # error here
train_rmse_2_a <- sqrt(sum((y_train_pred_2_a - data_train$medv)^2)/nrow(data_train))
test_rmse_2_a <- sqrt(sum((y_test_pred_2_a - data_test$medv)^2)/nrow(data_test)) # error here
...
y_test_pred_2_b <- pred(model_2_b, data = data_test) # error here
train_rmse_2_b <- sqrt(sum((y_train_pred_2_b - data_train$medv)^2)/nrow(data_train))
test_rmse_2_b <- sqrt(sum((y_test_pred_2_b - data_test$medv)^2)/nrow(data_test)) # error here
...
model_list <- list(library = "bonusLab",
                   ...
                   fit=function(x, y, wts, param, lev, last, weights, classProbs, ...) {
                     ridgereg(formula=as.formula(paste0(colnames(x)[14], "~ .")), data=x, lambda = param)
                   },
                   grid=data.frame(lambda = seq(0.1, 5, 0.1)),
                   predict=function(modelFit, newdata, preProc = NULL, submodels = NULL){
                     pred(modelFit, data=newdata)
                   },
                   prob=NULL
              )
cv_lambda_selection <- caret::train(x = data_train, 
                           y = data_train$medv, form = medv~.,
                           method=model_list,
                           tuneGrid = data.frame(lambda = seq(0.1, 5, 0.1)),
                           trControl = trainControl(# for 10-folds
                           number = 10,
                           repeats = 10))
cv_lambda_selection$results
```
Model 3:
```{r}
# model_3 <- ridgereg(medv ~ ., data_train, lambda = 2.3)
# y_train_pred_3 <- pred_reg(model_3)
# y_test_pred_3 <- pred_reg(model_3, data = data_test) # error here
# train_rmse_3 <- sqrt(sum((y_train_pred_3 - data_train$medv)^2)/nrow(data_train))
# test_rmse_3 <- sqrt(sum((y_test_pred_3 - data_test$medv)^2)/nrow(data_test)) # error here
# print(paste("Train RMSE is:", y_train_pred_3, "; Test RMSE is:", y_test_pred_3))
```
